...
namespace Evix.Voxel.Generation.Mesh {
  ...
  /// </summary>
  public class MarchGenerator : IBlockMeshGenerator {
    ...
    void march(Coordinate pointLocation) {
      ...
      foreach (Octants.Octant octant in Octants.All) {
        Coordinate blockVertexLocation = pointLocation + octant.Offset;
        blockVertexLocations[octant.Value] = blockVertexLocation;
        IBlockType blockType = points.getBlock(blockVertexLocation);

        // if this vertex is solid, mark it
        if (blockType.IsSolid) {
          blockMask = blockMask.SetVertexMaskForOctant(octant);
          // if the vertex isn't solid, and it represents this block's value(the first octant) skip it
        } else if (octant.Value == Octants.WestBottomSouth.Value) {
          return;
        }
      }
